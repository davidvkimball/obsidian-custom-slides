...
**Custom Slides** is an Obsidian plugin that enhances the built-in Slides core plugin with customization options. The plugin allows users to customize navigation controls, progress bar appearance, list alignment, and font settings for their presentations. It is designed for desktop-only use and requires the Slides core plugin to be enabled.

**Key Features**:
- **Navigation Control**: Toggle visibility of left, right, up, and down navigation arrows, plus the close button
- **Progress Bar Customization**: Adjust the height of the progress bar (default 10px, up from core plugin's 3px)
- **Left-Aligned Lists**: Enable left-aligned bulleted and numbered lists in presentation mode with proper spacing
- **Automatic Mode Switching**: Automatically switches to reading mode when entering Slides mode so keyboard input works immediately
- **Custom Font Support**: Use the same font settings as specified in Obsidian's text settings

**Status**: Production (version 1.0.7)

### Important Project-Specific Details

- **Project Type**: Plugin
- **Purpose**: Enhances the Obsidian Slides core plugin with customization options for navigation, styling, and behavior
- **Status**: Production
- **Key Features**:
  - Navigation arrow visibility toggles (left, right, up, down)
  - Close button visibility toggle
  - Progress bar height customization (default 10px)
  - Left-aligned list support with 1em left margin
  - Automatic reading mode switching when entering slides mode
  - Custom font support using Obsidian settings
- **Architecture**: 
  - Main plugin class: `CustomSlidesPlugin` in `src/main.ts`
  - Settings management: `src/settings.ts` with `CustomSlidesSettings` interface
  - Settings tab: `src/ui/settings-tab.ts` using SettingGroup pattern
  - Mode observer: `src/utils/mode-observer.ts` for automatic mode switching
  - Settings compatibility: `src/utils/settings-compat.ts` for backward-compatible SettingGroup support
  - Dynamic CSS: Applied via body classes and CSS custom properties

### Maintenance Tasks

- Keep settings compatibility utility updated if Obsidian API changes
- Monitor for Obsidian Slides core plugin API changes
- Update version numbers via `npm version` command
- Sync reference materials periodically (optional)
- Test compatibility with new Obsidian versions, especially when SettingGroup API evolves

### Project-Specific Conventions

- Settings use boolean toggles and numeric inputs
- Dynamic styles applied via body class toggles (`hide-navigate-left`, `hide-navigate-right`, etc.) and CSS custom properties (`--progress-height`)
- Mode observer pattern for automatic reading mode switching using MutationObserver
- Settings compatibility utility for backward-compatible SettingGroup usage (API 1.11.0+)
- All settings grouped in a single SettingGroup with no heading (following UI Tweaker pattern)
- Error handling uses silent try-catch blocks in critical paths

### Project-Specific References

- `.ref/plugins/obsidian-ui-tweaker/` - Reference implementation for SettingGroup usage and settings compatibility patterns. Used as a guide for implementing backward-compatible SettingGroup support.
...
- `src/main.ts` - Main plugin class (`CustomSlidesPlugin`), handles settings loading/saving, dynamic style application, mode observer setup, and settings tab registration
- `src/settings.ts` - Settings interface (`CustomSlidesSettings`) and default values (`DEFAULT_SETTINGS`)
- `src/ui/settings-tab.ts` - Settings UI implementation using SettingGroup pattern for proper spacing and organization
- `src/utils/mode-observer.ts` - Observes DOM for slides mode entry/exit using MutationObserver, automatically switches to reading mode when entering slides and restores previous mode on exit
- `src/utils/settings-compat.ts` - Settings compatibility utility for backward-compatible SettingGroup usage (API 1.11.0+), provides fallback for older Obsidian versions
- `styles.css` - CSS styles for hiding navigation elements (via body classes) and customizing progress bar (via CSS custom properties)
- `manifest.json` - Plugin manifest (desktop-only: `isDesktopOnly: true`, minAppVersion 0.15.0)

### Development Notes

- **Desktop-only plugin**: `isDesktopOnly: true` in manifest - does not support mobile
- **Requires Slides core plugin**: Plugin depends on Obsidian's built-in Slides core plugin being enabled
- **Dynamic CSS approach**: Uses body class toggles (`hide-navigate-left`, `hide-navigate-right`, `hide-navigate-up`, `hide-navigate-down`, `hide-close-btn`, `left-align-bullets`) and CSS custom properties (`--progress-height`, `--slides-font-interface`, `--slides-font-text`)
- **Mode switching**: Automatically switches to reading mode when entering slides mode (so spacebar and arrows work immediately), restores previous mode (e.g., edit or source) when exiting slides mode
- **Settings compatibility**: Uses `createSettingsGroup()` utility for backward-compatible SettingGroup support (API 1.11.0+), with fallback for older versions
- **Reference pattern**: Follows UI Tweaker's implementation for SettingGroup usage - first group has no heading, all settings in a single group
- **Error handling**: Uses silent error handling in `onload()` and mode observer to prevent plugin crashes from affecting Obsidian stability
